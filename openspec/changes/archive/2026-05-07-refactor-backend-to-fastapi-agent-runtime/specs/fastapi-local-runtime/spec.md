...
系统 SHALL 使用 FastAPI 提供本地 HTTP API，并保留个人本地应用的单入口启动体验。
...
#### Scenario: 新增 API 能力
- **WHEN** 开发者新增一个后端能力
- **THEN** 系统 SHALL 将路由放在 `backend/api/`，业务逻辑放在 `backend/services/`，持久化逻辑放在 `backend/repositories/`

#### Scenario: 校验请求和响应
- **WHEN** API 接收请求或返回结构化响应
- **THEN** 系统 SHALL 使用 Pydantic schema 描述关键字段和默认值
